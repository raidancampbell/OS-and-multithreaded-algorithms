...
char *argv[];
{
	...
	if (getsockname(0, (struct sockaddr *)&saddr, &asize) == 0) {
		...
		_rpcpmstart = 1;
		proto = 0;
		openlog("MailBox", LOG_PID, LOG_DAEMON);
	} else {
#ifndef RPC_SVC_FG
		int size;
		int pid, i;
	...
	if ((_rpcfdtype == 0) || (_rpcfdtype == SOCK_DGRAM)) {
		...
		if (!_rpcpmstart)
			proto = IPPROTO_UDP;
		if (!svc_register(transp, DISPLAY_PRG, DISPLAY_VER, display_prg_1, proto)) {
			_msgout("unable to register (DISPLAY_PRG, DISPLAY_VER, udp).");
			exit(1);
		}
	}
	...
	if ((_rpcfdtype == 0) || (_rpcfdtype == SOCK_STREAM)) {
		...
		if (!_rpcpmstart)
			proto = IPPROTO_TCP;
		if (!svc_register(transp, DISPLAY_PRG, DISPLAY_VER, display_prg_1, proto)) {
			_msgout("unable to register (DISPLAY_PRG, DISPLAY_VER, tcp).");
			exit(1);
		}
	}

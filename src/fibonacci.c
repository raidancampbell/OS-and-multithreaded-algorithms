...
/**
* safely prints the username of the user executing this thread
*/
void printLogin(){
    char * username = getlogin();
    if(username == NULL) perror("ERROR WHILE GETTING USERNAME");
    printf("Login: %s\n",username);
    fflush(stdout);
}
...
void printProcessStats(time_t startTime) {
    ...
    if(getrusage(RUSAGE_SELF, &rusage1 ) == -1) perror("ERROR WHILE GETTING RESOURCE USAGE");
    ...
    printf("Process took %.2f seconds to execute\n", difftime(get_time,startTime));
